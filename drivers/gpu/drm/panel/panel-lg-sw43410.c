...
/*
 * LG SW43410 MIPI DSI panel used by the LM-G820N.
 *
 * Initialization tables come from the stock SW43410 MP panel definition.
 */
...
disable_vddi:
	/* Keep VDDI on if VPNL could not be disabled in the previous cycle. */
	if (ctx->vpnl_enabled)
		goto sleep_pinctrl;
	regulator_set_load(ctx->supplies[1].consumer, 0);
	regulator_set_load(ctx->supplies[0].consumer, 80);
	cleanup_ret = regulator_disable(ctx->supplies[0].consumer);
	if (!cleanup_ret)
		ctx->vddi_enabled = false;
	else
		dev_err(&ctx->link->dev, "Failed to disable VDDI: %d\n", cleanup_ret);
	goto sleep_pinctrl;
...
sleep_pinctrl:
	...
	cleanup_ret = pinctrl_pm_select_sleep_state(&ctx->link->dev);
	if (cleanup_ret)
...
static void sw43410_disable_locked(struct sw43410_panel *ctx)
{
...
disable:
	/* The panel core skips disable() when enable() fails. */
	sw43410_disable_locked(ctx);
unlock:
	mutex_unlock(&ctx->lock);
	return ret;
...
static int sw43410_prepare(struct drm_panel *panel)
{
	...
	mutex_lock(&ctx->lock);
	ret = ctx->removing ? -ENODEV : sw43410_power_on(ctx);
	mutex_unlock(&ctx->lock);
...
static int sw43410_probe(struct mipi_dsi_device *dsi)
{
	...
	/* The stock command-mode configuration does not force the clock lane HS. */
	dsi->mode_flags = MIPI_DSI_MODE_LPM |
...
static void sw43410_remove(struct mipi_dsi_device *dsi)
{
	...
	ctx->removing = true;
	ctx->ready = false;
	if (ctx->vpnl_enabled || ctx->vddi_enabled) {
		gpiod_set_value_cansleep(ctx->reset_gpio, 1);
		ret = sw43410_power_off(ctx);
		if (ret)
			dev_err(&dsi->dev, "Failed to power off panel: %d\n", ret);
	}
	mutex_unlock(&ctx->lock);
