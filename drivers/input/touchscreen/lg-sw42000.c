...
struct sw42000 {
	...
	/* Serialize panel and system power transitions. */
	struct mutex lock;
	bool powered;
	bool active;
	bool panel_prepared;
...
static int sw42000_start(struct sw42000 *ts)
{
	...
	if (version[2] != 10 || (version[3] & 0x0f) != 4) {
		dev_err(&ts->client->dev,
			"Unrecognized controller: chip=%u protocol=%u\n",
	...
	dev_dbg(&ts->client->dev, "SW42000 v%u.%02u started\n",
...
disable:
	gpiod_set_value_cansleep(ts->reset_gpio, 1);
	if (enabled_supplies) {
		disable_error = regulator_bulk_disable(ARRAY_SIZE(ts->supplies),
						       ts->supplies);
		if (disable_error)
...
static int sw42000_deep_sleep(struct sw42000 *ts)
{
	...
	error = sw42000_write_u32(ts, SW42000_REG_INTERNAL_CLOCK, 0);
	if (error)
		goto reset;
...
reset:
	/* The panel will power down even if its follower reports an error. */
	gpiod_set_value_cansleep(ts->reset_gpio, 1);
	return error;
}
...
static int sw42000_probe(struct i2c_client *client)
{
	...
	/* Handle initial activation errors in probe, not in the follower callback. */
	ts->suspended = true;
	...
	/* Remove the follower and stop the controller before releasing the IRQ. */
	...
	if (ts->panel_prepared) {
		error = sw42000_activate(ts);
		if (error)
			return dev_err_probe(dev, error, "Failed to start controller\n");
	}
...
static int sw42000_suspend(struct device *dev)
{
	...
	if (ts->panel_prepared) {
		...
		if (resume_error)
			dev_err(dev, "Failed to recover from suspend failure: %d\n",
				resume_error);
	}
