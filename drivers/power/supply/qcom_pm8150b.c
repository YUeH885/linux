			      ...
			      int *current_max_ua)
{
	...
	/* Availability must not depend on our own USBIN suspend state. */
	ret = pm8150b_usb_present(chip, &present);
	if (ret || !present) {
...
static int pm8150b_set_usb_icl(struct pm8150b_charger *chip, int current_ua)
{
	...
	if (current_ua == chip->applied_icl_ua)
		return 0;
	/* A failed multi-register update must not leave a valid cached limit. */
	chip->applied_icl_ua = -1;
...
static int pm8150b_disable_parallel(struct pm8150b_charger *chip)
{
	...
	if (!chip->parallel_enabled && !chip->parallel_uncertain)
...
static int pm8150b_set_charging(struct pm8150b_charger *chip, bool enable)
{
	unsigned int cmd;
	int ret;
	...
	ret = regmap_update_bits(chip->regmap, PM8150B_CHGR_ENABLE_CMD,
				 ...
				 enable ? PM8150B_CHARGING_ENABLE : 0);
	if (!ret) {
		ret = regmap_read(chip->regmap, PM8150B_CHGR_ENABLE_CMD, &cmd);
		if (!ret && !!(cmd & PM8150B_CHARGING_ENABLE) != enable)
			ret = -EIO;
	}
	chip->charging_state_valid = !ret;
	if (!ret)
		chip->charging_enabled = enable;
	return ret;
}

static void pm8150b_stop_charging(struct pm8150b_charger *chip)
{
	int main_ret, parallel_ret, input_ret;

	main_ret = pm8150b_set_charging(chip, false);
	parallel_ret = pm8150b_disable_parallel(chip);
	input_ret = pm8150b_set_usb_icl(chip, PM8150B_USB_ICL_UNKNOWN_UA);
	if (main_ret || parallel_ret || input_ret)
		dev_err_ratelimited(chip->dev,
			"Failed to stop charging: main=%d parallel=%d input=%d\n",
			main_ret, parallel_ret, input_ret);
}
				...
				struct pm8150b_policy_target *target)
{
	...
	/* Hardware controls single-path CV; software taper only sheds parallel FCC. */
	if (!chip->parallel_enabled && !chip->taper_active)
		return;
	if (s->phase == PM8150B_TAPER_CHARGE) {
		...
		if (chip->parallel_enabled &&
		    time_after_eq(jiffies, chip->taper_next_step)) {
				...
				bool allow_parallel)
{
	...
	if (target->disable_charging) {
		ret = pm8150b_set_charging(chip, false);
		if (ret)
			return ret;
	}
	 ...
	 */
	parallel_fcc_ua = pm8150b_parallel_fcc(total_fcc_ua, settled_icl_ua,
					    total_icl_ua, settled_voltage_uv,
					    fv_uv, chip->parallel_percent);
	...
	/*
	 * Lower the shrinking side first so the combined FCC never exceeds the
	...
	if (ret)
		goto parallel_failed;
	/* Renew the secondary watchdog lease only after applying all limits. */
	chip->parallel_uncertain = true;
	...
	return pm8150b_set_charging(chip, true);

parallel_failed:
	dev_warn_ratelimited(chip->dev, "Parallel path failed: %d\n",
		 ret);
	chip->parallel_uncertain = true;

single:
	/* Full main FCC is allowed only after confirming the secondary gate. */
	...
	if (ret)
		return ret;
	/* Program limits before reopening the charging gate. */
	return pm8150b_set_charging(chip, !target->disable_charging);
}
					...
					struct pm8150b_policy_snapshot *s)
{
	...
	ret = regmap_read(chip->regmap, PM8150B_CHGR_STATUS_2, &status2);
	if (ret)
		return ret;
	s->battery_overvoltage = status2 & PM8150B_CHGR_BAT_OV;
	ret = regmap_read(chip->regmap, PM8150B_BATIF_INT_RT_STS, &batif);
	if (ret)
		return ret;
				     ...
				     struct pm8150b_policy_target *target)
{
	...
	target->suspend_input = !s->present || !chip->typec_online;
	...
	if (!s->present || !chip->typec_online || s->battery_temp_limited ||
	    s->battery_missing || s->battery_overvoltage)
		target->disable_charging = true;
	...
	target->input_current_ua = clamp(target->input_current_ua, 0,
	...
	if (s->usb_type == POWER_SUPPLY_USB_TYPE_SDP &&
	    pm8150b_use_gadget_limit(chip->typec_online, chip->typec_current_max_ua)) {
		...
		if (!chip->gadget_current_limit_valid)
			...
			gadget_limit = clamp(gadget_limit,
					     0,
					     PM8150B_USB_ICL_FAST_5V_UA);
		target->input_current_ua = min(target->input_current_ua, gadget_limit);
		target->input_current_ua = rounddown(target->input_current_ua,
						     PM8150B_USB_ICL_STEP_UA);
	}
...
static void pm8150b_policy_work(struct work_struct *work)
{
	...
	/* Feeding is conditional on a complete, successful policy update. */
	if (!ret)
		ret = regmap_write(chip->regmap, PM8150B_WDOG_PET,
				   PM8150B_WDOG_PET_BIT);
	if (ret)
		pm8150b_stop_charging(chip);
	...
	if (ret) {
		delay_ms = PM8150B_POLICY_NORMAL_MS;
		dev_err_ratelimited(chip->dev, "Failed to update charging policy: %d\n", ret);
	}
	present = chip->input_present;
...
static int pm8150b_read_typec_supply(struct pm8150b_charger *chip,
				    struct power_supply *psy)
{
	...
	if (chip->pd_active) {
		if (voltage.intval != PM8150B_USB_VOLTAGE_5V_UV &&
		    voltage.intval != PM8150B_USB_VOLTAGE_9V_UV)
			return -ERANGE;
...
{
	struct pm8150b_charger *chip = container_of(nb, struct pm8150b_charger, psy_nb);
	struct power_supply *psy = data;

	if (event != PSY_EVENT_PROP_CHANGED || !psy->dev.parent ||
	    dev_fwnode(psy->dev.parent) != chip->typec_fwnode)
		return NOTIFY_DONE;
	/* The worker owns hardware writes and reads a fresh TCPM snapshot. */
	mutex_lock(&chip->usb_lock);
	if (!chip->shutting_down)
...
static int pm8150b_sync_typec_supply(struct pm8150b_charger *chip)
{
	...
	ret = pm8150b_read_typec_supply(chip, psy);
...
static int pm8150b_battery_health(struct pm8150b_charger *chip, int *health)
{
	...
	if (val & PM8150B_CHGR_BAT_OV) {
		...
		if (ret)
			return ret;
		if (voltage >= chip->constant_charge_voltage_max_uv + 40000) {
					...
					union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CONSTANT_CHARGE_VOLTAGE_MAX:
		ret = regmap_read(chip->regmap, PM8150B_CHGR_FV_CFG, &reg);
		if (!ret)
			val->intval = PM8150B_FV_MIN_UV + reg * PM8150B_FV_STEP_UV;
		return ret;
	default:
		return -EINVAL;
	}
...
static int pm8150b_load_battery_info(struct pm8150b_charger *chip)
{
	...
	if (chip->charge_full_design_uah <= 0 ||
	    chip->charge_full_design_uah > INT_MAX / 100 ||
	    chip->constant_charge_current_max_ua < PM8150B_FCC_STEP_UA ||
	    chip->constant_charge_current_max_ua > 8000000 ||
	    chip->constant_charge_voltage_max_uv < PM8150B_FV_MIN_UV ||
	    chip->constant_charge_voltage_max_uv > 4790000)
		return -EINVAL;
...
static void pm8150b_stop(void *data)
{
	...
	mutex_lock(&chip->usb_lock);
	pm8150b_stop_charging(chip);
	if (chip->charging_state_valid && !chip->charging_enabled) {
		ret = regmap_update_bits(chip->regmap, PM8150B_WDOG_CFG,
			PM8150B_WDOG_TRIGGER_AFP | PM8150B_WDOG_BARK_IRQ_ENABLE |
			PM8150B_WDOG_ENABLE_ON_PLUGIN, 0);
		if (ret)
			dev_err(chip->dev, "Failed to stop charger watchdog: %d\n", ret);
	}
	mutex_unlock(&chip->usb_lock);
}
...
static int pm8150b_probe(struct platform_device *pdev)
{
	...
	/* Published supplies and IRQs must not schedule policy during probe. */
	chip->shutting_down = true;
	chip->usb_voltage_max_uv = PM8150B_USB_VOLTAGE_5V_UV;
	chip->gadget_current_limit_ua = -1;
	chip->gadget_current_limit_valid = false;
	...
	if (ret)
		return dev_err_probe(chip->dev, ret, "Failed to read battery parameters\n");
	ret = pm8150b_clamp_profile(chip);
	if (ret)
		return dev_err_probe(chip->dev, ret,
				     "Failed to clamp battery charging limits\n");
	...
	ret = devm_add_action_or_reset(chip->dev, pm8150b_stop, chip);
	if (ret)
		return ret;
	chip->parallel_uncertain = true;
	ret = pm8150b_disable_parallel(chip);
	...
	dev_info(chip->dev,
		 "PM8150B power_supply enabled, battery limits %d uA/%d uV, %d%% recharge\n",
		 chip->constant_charge_current_max_ua,
		 chip->constant_charge_voltage_max_uv,
